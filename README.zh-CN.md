...
会拒绝其他状态。项目可以启用失败关闭的投稿门槛；项目模板默认要求修订后由用户
导出的知网、Turnitin 或 iThenticate 全文报告不高于 `10%`，并同时绑定修订稿哈希、
报告哈希、复核人和时间。高于门槛、报告过期或只提供单条命中比例时均阻止进入
Word/WPS。报告中的平台标识必须与声明一致，且报告不得早于当前修订稿；同一平台
的新报告会原子替换旧报告并立即清除旧批准，避免 WPS/Word 误用过期的
`qa_passed`。该数值只是发布政策，不是对任何论文未来查重结果的保证，也不允许
规避检测式改写。详见
